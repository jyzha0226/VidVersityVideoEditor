...
export default function HomePage(): JSX.Element {
  ...
  const keepPlaybackWithinClipPreview = (video: HTMLVideoElement, clip: TrimClip): void => {
    const orderedSegments = getOrderedSegments(clip)
    if (!orderedSegments.length) return
              ...
              {videoUrl ? (
                <video
                  ...
                  onTimeUpdate={(event) => {
                    ...
                    setCurrentTime(now)
                    if (activeClip) {
                      keepPlaybackWithinClipPreview(video, activeClip)
                    }
                  }}
                  onEnded={(event) => {
                    if (activeClip) {
                      resetClipPlayback(event.currentTarget, activeClip)
                    }
                  }}
                />
              ) : (
