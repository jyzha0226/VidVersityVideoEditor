...
export default function HomePage(): JSX.Element {
  ...
  const syncPlaybackToClipWindow = (video: HTMLVideoElement, clip: TrimClip): void => {
    const orderedSegments = getOrderedSegments(clip)
    if (!orderedSegments.length) return
    ...
    if (now < firstSegment.start - epsilon) {
      video.currentTime = firstSegment.start
      return
    }

    if (now > lastSegment.end + epsilon) {
      resetClipPlayback(video, clip)
      return
    }

    const currentSegmentIndex = orderedSegments.findIndex(
              ...
              {videoUrl ? (
                <video
                  ...
                  onPlay={(event) => {
                    ...
                    syncPlaybackToClipWindow(event.currentTarget, activeClip)
                  }}
                  onSeeking={(event) => {
                    if (!activeClip) return
                    syncPlaybackToClipWindow(event.currentTarget, activeClip)
